...
const SavedPosts = () => {
    ...
    const fetchPosts = async (pageNum = 1) => {
        ...
        try {
            const res = await axiosClient.get(`/posts/saved?page=${pageNum}&limit=5`);
            const newPosts = res.data.data || [];
            // Đối với API saved, gán mặc định is_saved = true
            const formattedPosts = newPosts.map(p => ({...p, is_saved: true}));
            if (pageNum === 1) setPosts(formattedPosts); else setPosts(prev => [...prev, ...formattedPosts]);
    ...
    return (
        <div className="max-w-2xl mx-auto mt-4 px-4 space-y-8 pb-10">
            <h1 className="text-2xl font-bold text-stone-800 dark:text-stone-100 mb-6">Bài viết đã lưu</h1>
